...
introduced_bills <- function(input_df) {
  ...
  # Get every entry within the list, taking only the bill's URL from the <a> node.
  bill_list_html <- as.list(bill_list_page_html %>%
    html_nodes("ul.linkSect > li > a") %>%
    html_attr("href"))
  
  new_list <- paste0(site_url, bill_list_html)
  
  # Bills introduced in a session are shown 80 per page.
  # If there are more bills, the word "More..." appears
  # next to a hyperlink containing "ALL". If the last entry
  # in the list contains "ALL"
  #604.exe?941+lst+ALL+HB0080"
