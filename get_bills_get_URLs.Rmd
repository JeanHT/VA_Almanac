...
Building a list of the General Assembly sessions available through LIS will enable the creation of functions to build lists of URLS to traverse to read the text of bills.
...
# The list of sessions needs to be cleaned up to turn it into an iterable list.
...
ga_sessions <- str_split(ga_sessions_html, "\r\n") %>%
  ...
  sort(ga_sessions, decreasing = FALSE) %>%
  # Remove the empty element, anartifact leftover from str_split
  `[` (-1:-1)
```
...
```
#1994 example
#http://lis.virginia.gov/cgi-bin/legp604.exe?941+ful+HB1
#2018 example
#http://lis.virginia.gov/cgi-bin/legp604.exe?181+ful+HB1
...
Not all types may be introduced in every session. In addition to the annual session, the General Assembly may also convene one or more special ga_sessions.
