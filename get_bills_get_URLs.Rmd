...
Building a list of the General Assembly sessions available through LIS will enable us to build lists of URLS to traverse to read the text of bills.
...
Scrape the LIS Web site to figure out how many bills were presented and ordered printed in each session. The list of bills is contained in an unordered list of class "linkSect."

```{r function_get_bill_list}
bill_list <- function() {
  # This function is under construction
  
  # Add error handling later
  #bill_list_page_html <- read_html(bill_list_url)
  print(session_bill_list_url)
  
  #ga_sessions_html <- bill_list_page_html %>%
  #html_node("#sLink") %>%
  #html_text()
  
  # Read the sessions bill list Web page
  #page_html <- read_html(bill_list_url)
}
```
...
select_years <- function(year_start, year_end) {
  ...
  sessions_df <- subset(ga_sessions_df, 
                        session_year >= year_start & session_year <= year_end)
  print(sessions_df)
}
```
